...
export default function TeacherSessionsDashboard() {
  ...
  // Grading Modal State for Instructors
  ...
  const handleSaveGrade = (e: React.FormEvent) => {
    ...
    store.gradeProject(gradingProject.id, numScore, noteInput);

    if (selectedMeetingId) {
      loadProjectsForMeeting(selectedMeetingId);
    }
    setGradeSuccessToast(`Nilai untuk ${gradingProject.student?.full_name || gradingProject.name} berhasil disimpan! (${numScore}/100)`);
    setTimeout(() => setGradeSuccessToast(null), 3000);
  ...
  const handleRemoveGrade = () => {
    ...
    setGradeSuccessToast(`Nilai untuk ${gradingProject.student?.full_name || gradingProject.name} telah direset.`);
    setTimeout(() => setGradeSuccessToast(null), 3000);
    setGradingProject(null);
  };
    ...
    <div className="min-h-screen bg-[#fafbfc] dark:bg-[#07080c] text-slate-900 dark:text-slate-100 transition-colors pb-12 font-sans relative selection:bg-primary selection:text-white">
      ...
      {gradeSuccessToast && (
        <div className="fixed top-6 right-6 z-50 bg-emerald-600 text-white px-4 py-3 rounded-2xl shadow-xl flex items-center gap-3 border border-emerald-400/30 animate-in fade-in slide-in-from-top-4 duration-200">
          ...
          <span className="text-xs md:text-sm font-bold">{gradeSuccessToast}</span>
        </div>
      )}
                            ...
                            <div className="space-y-2.5 min-w-0">
                              ...
                              <div className="flex items-center justify-between gap-2 min-w-0">
                                ...
                                <div className="flex items-center gap-1.5 shrink-0">
                                  {/* Instructor Score Badge */}
                                  {isTeacher && !isTeacherOwner && (
                                    project.score !== undefined && project.score !== null ? (
                                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-lg bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20 flex items-center gap-1">
                                        <Star className="w-3 h-3 fill-emerald-500 text-emerald-500 shrink-0" />
                                        <span>{project.score} / 100</span>
                                      </span>
                                    ) : (
                                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-lg bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/30 flex items-center gap-1">
                              ...
                              {/* Student's Own Grade Display (Private to student) */}
                              {!isTeacher && isMyProject && (
                                project.score !== undefined && project.score !== null ? (
                                  <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-xs space-y-1.5 animate-in fade-in duration-200">
                                    <div className="flex items-center justify-between font-bold text-emerald-700 dark:text-emerald-400">
                                      <span className="flex items-center gap-1.5">
                                        <Award className="w-4 h-4 text-emerald-600" />
                                        <span>Nilai Kamu: {project.score} / 100</span>
                                      </span>
                                      {project.graded_at && (
                                        <span className="text-[10px] text-slate-400 font-normal font-mono">
                                          {new Date(project.graded_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}
                                        </span>
                                      )}
                                    </div>
                                  ...
                                  <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/25 text-[11px] font-semibold text-amber-700 dark:text-amber-400">
                                    <Clock className="w-3.5 h-3.5 shrink-0" />
                                    <span>Tugas ini sedang menunggu penilaian guru</span>
                                  </div>
                                )
                              )}
                            </div>

                            {/* Action Buttons: Live Preview, Grade & Source Code */}
                              ...
                              <div className="flex items-center gap-1.5 shrink-0">
                                ...
                                {isTeacher && !isTeacherOwner && (
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleOpenGradeModal(project);
                                    }}
                                    className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all border min-h-[36px] ${
                                      project.score !== undefined && project.score !== null
                                    ...
                                    title="Beri atau edit nilai untuk folder siswa ini"
                                  >
                                    <Award className="w-3.5 h-3.5 shrink-0" />
                                    <span>{project.score !== undefined && project.score !== null ? `${project.score}/100` : 'Beri Nilai'}</span>
                                  </button>
                                )}
